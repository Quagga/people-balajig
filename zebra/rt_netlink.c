  ...
  if (rtm->rtm_family == AF_INET)
    {
      ...
      if (h->nlmsg_type == RTM_NEWROUTE)
        rib_add_ipv4 (ZEBRA_ROUTE_KERNEL, 0, &p, gate, src, index, table, 0, 0, SAFI_UNICAST);
      else
  ...
  if (rtm->rtm_family == AF_INET6)
    {
      ...
      if (h->nlmsg_type == RTM_NEWROUTE)
        rib_add_ipv6 (ZEBRA_ROUTE_KERNEL, 0, &p, gate, index, 0, 0, 0, SAFI_UNICAST);
      else
        rib_delete_ipv6 (ZEBRA_ROUTE_KERNEL, 0, &p, gate, index, 0, SAFI_UNICAST);
    }
#endif /* HAVE_IPV6 */
